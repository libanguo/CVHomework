    ...
    if evaluation.video == 'all':
        ...
        for i in range(nv):
            ...
            for j in range(1):
                ...
                path=os.path.join(env.root_dataset, evaluation.dataset, videos_list[i])
                path=path+"/groundtruth1.txt"
                print(videos_list[i])
                f=open(path,'w')
                print(len(bboxes))
                for k in range(len(bboxes)):
                    x1=bboxes[k][0]-bboxes[k][2]/2
                    y1=bboxes[k][1]-bboxes[k][3]/2
                    x2=bboxes[k][0]+bboxes[k][2]/2
                    y2=bboxes[k][1]+bboxes[k][3]/2
                    f.writelines(str(x1)+","+str(y1)+","+str(x2)+","+str(y1)+","+str(x2)+","+str(y2)+","+str(x1)+","+str(y2)+"\n")
                f.close()
                # f=open()
